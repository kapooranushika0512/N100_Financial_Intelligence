...
def show():
    ...
    c1.metric(
        "Average ROE",
        f"{merged['return_on_equity_pct'].mean():.2f}%"
    )

    c2.metric(
        "Median Debt / Equity",
        f"{merged['debt_to_equity'].median():.2f}"
    )

    c3.metric(
        "Average Net Profit Margin",
        f"{merged['net_profit_margin_pct'].mean():.2f}%"
    )

    c4.metric(
        "Average Asset Turnover",
        f"{merged['asset_turnover'].mean():.2f}"
    )

    c5.metric(
        "Average Interest Coverage",
        f"{merged['interest_coverage'].mean():.2f}"
    )

    debt_free = (
        merged["debt_to_equity"] <= 0
    ).sum()
